...
> **Safe, autonomous, sandboxed Google Antigravity CLI (`agy`) inside an ephemeral Docker container with persistent authentication and zero host file permission issues.**
...
**agyInABasket** encapsulates `agy` inside an isolated Ubuntu-based Docker container:
...
- **Persistent Auth & Sessions:** Google OAuth tokens, conversation logs, and settings are saved into a dedicated Docker volume (`agy-data`), so you log in once and stay authenticated across all projects and sessions.
...
```mermaid
flowchart TD
    subgraph Host["Host Machine (Linux Mint / Ubuntu)"]
        User["User Terminal"]
        Launcher["aiab (CLI / Shell Function)"]
        Proj["Local Project Folder (/path/to/my-project)"]
        DockerVol["Docker Volume: agy-data (~/.gemini)"]
    end
    ...
    User -->|"aiab ."| Launcher
    Launcher -->|"docker run -it --rm"| Container
...
### 1. Prerequisites
- Linux host (tested on Linux Mint and Ubuntu).
- Docker installed and running.
- Your host user added to the `docker` group:
  ```bash
